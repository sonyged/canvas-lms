...
function renderQuizAttributes() {
  ReactDOM.render(
    <QuizAttributeSelection
      subjectOptions={ENV.subject_options}
...
$(document).ready(function () {
  ...
  if (!ENV.use_rce_enhancements) {
    ...
    $('.toggle_question_content_views_link').click(function (event) {
      ...
      RichContentEditor.callOnRCE(
        $(this).parents('.question_form').find('.question_content'),
        'toggle'
      )
      //  todo: replace .andSelf with .addBack when JQuery is upgraded.
    ...
    $('.toggle_text_after_answers_link').click(function (event) {
      event.preventDefault()
      RichContentEditor.callOnRCE(
        $(this).parents('.question_form').find('.text_after_answers'),
        'toggle'
      )
      //  todo: replace .andSelf with .addBack when JQuery is upgraded.
  ...
  function showQuizUnitOptions() {
    ...
    for (const unit of all_units.get()) {
        ...
        variables.forEach((variable, index) => {
          constraints.forEach(constraint => {
            if (variable.name === constraint.var1) {
              ...
              if (rule === '=*') {
                if (
                  variable.value % var2.value !== 0 ||
                  variable.value / var2.value < constraint.min ||
                  variable.value / var2.value > constraint.max
                ) {
                  hasUpdate = true
                  var2.value = getRandomInt(var2.value - var2.min) + var2.min
                  let mul = getRandomInt(constraint.max - constraint.min) + constraint.min
                  if (mul > Math.floor(variable.max / var2.value))
                    mul = Math.floor(variable.max / var2.value)
